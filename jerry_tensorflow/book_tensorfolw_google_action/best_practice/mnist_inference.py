...
# 保存的模型加载这边变量的取值。而且更加方面的是，因为可以在变量加载时将滑动平均变量
# 重命名，所以可以直接通过他那个样的名字在训练时使用变量自身，而在测试时使用变量的滑
# 动平均值。在这个函数中也会将变量的正则化损失加入损失集合。
def get_weight_variable(shape, regularizer):
    weights = tf.get_variable('weights', shape,
                              initializer=tf.truncated_normal_initializer(stddev=0.1))
    # 当给出了正则化生成函数时，将当前变量的正则化损失加入名字为losses的集合。在这里
    # 使用了add_to_collection函数将一个张量加入一个集合，而这个集合的名称诶losses。
    if regularizer != None:
...
def inference(input_tensor, regularizer):
    ...
    with tf.variable_scope('layer1'):
        ...
        # 在统一个程序中多次调用这个函数。如果在同一个程序中多次调用，在第一次调用
        # 只有需要将reuse参数设置为True
