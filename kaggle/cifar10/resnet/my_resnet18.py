...
def Conv2d_BN(x, nb_filter, kernel_size, strides=(1, 1),
              padding='same'):
    x = Conv2D(nb_filter, kernel_size=kernel_size, strides=strides, padding=padding, activation='relu'
...
if __name__ == '__main__':
    model = resnet_18(IM_WIDTH, IM_HEIGHT, 3, NB_CLASS)
    model.summary()
    ...
    def lr_scheduler(epoch):
        return learning_rate * (0.914 ** (epoch // lr_drop))

    reduce_lr = keras.callbacks.LearningRateScheduler(lr_scheduler)
    optimizer = keras.optimizers.SGD(learning_rate, 0.9, 1e-7, nesterov=True)
    ...
    file.write(
        '\nepochs %d sec_cost %.2f model resnet18 momentum decay 0.914 ** (epoch // 3) datagen datagen\n' % ( \
